...
class HypMLR(nn.Module):
    def __init__(self, 
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=32, kernel_size=3, padding=1).double(), # 28x28x32
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2), # 14x14x32
            
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, padding=1).double(), # 14x14x64
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)  # =>7x7x64
        )

        self.fc = nn.Linear(7 * 7 * 64, hyp_dim).double() # => hyp_dim

        self.output = nn.Linear(hyp_dim, class_num).double()

        self.layer_norm = nn.LayerNorm(hyp_dim).double() 

        self.poincare_ball = PoincareBall(c=c, bound_eps=bound_eps)
        self.hyperplane_normals = nn.Parameter(torch.randn(class_num, hyp_dim, dtype=torch.float64)) # euclidean, (classnum, hyp_dim) = (C, D)
        self.hyperplane_offsets = nn.Parameter(torch.zeros(class_num, hyp_dim, dtype=torch.float64)) # hyperbolic, (C, D)
